# Görev: 23. Söz - 2. Mebhas - 1. Nükte video üretim scaffold dosyalarını doldur
...
```bash
python3 -m json.tool docs/video-tests/inputs/23-soz-2-mebhas-1-nukte-landscape-full-video.json > /tmp/23-soz-2-mebhas-1-nukte-landscape-json-check.json
python3 -m json.tool docs/video-tests/metadata/23-soz-2-mebhas-1-nukte-landscape-metadata.json > /tmp/23-soz-2-mebhas-1-nukte-landscape-metadata-check.json
python3 -m json.tool docs/video-tests/shorts/23-soz-2-mebhas-1-nukte/23-soz-2-mebhas-1-nukte-shorts-package.json > /tmp/23-soz-2-mebhas-1-nukte-shorts-package-check.json
python3 -m json.tool docs/video-tests/shorts/23-soz-2-mebhas-1-nukte/metadata/23-soz-2-mebhas-1-nukte-shorts-metadata.json > /tmp/23-soz-2-mebhas-1-nukte-shorts-metadata-check.json
```

JS syntax için:

```bash
node -e "new Function(require('fs').readFileSync('docs/video-tests/inputs/23-soz-2-mebhas-1-nukte-landscape-load-input.js', 'utf8')); console.log('Landscape JS OK')"

for f in docs/video-tests/shorts/23-soz-2-mebhas-1-nukte/load-inputs/*.js; do
...
```bash
grep -RniE "bu metin|metnin|metinde|metne göre|metnin deyimiyle|bu noktada|şu cümle|verilen cevap|verilen örnek|yazar burada|kitap burada|burada anlatılmak istenen|bu bölüm" docs/video-tests/inputs/23-soz-2-mebhas-1-nukte* docs/video-tests/shorts/23-soz-2-mebhas-1-nukte || true
```
