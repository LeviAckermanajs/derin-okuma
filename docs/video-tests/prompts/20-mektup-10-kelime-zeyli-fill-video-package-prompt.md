# Görev: Bir Zerre Ne Yapabilir video üretim scaffold dosyalarını doldur
...
```bash
python3 -m json.tool docs/video-tests/inputs/20-mektup-10-kelime-zeyli-landscape-full-video.json > /tmp/20-mektup-10-kelime-zeyli-landscape-json-check.json
python3 -m json.tool docs/video-tests/metadata/20-mektup-10-kelime-zeyli-landscape-metadata.json > /tmp/20-mektup-10-kelime-zeyli-landscape-metadata-check.json
python3 -m json.tool docs/video-tests/shorts/20-mektup-10-kelime-zeyli/20-mektup-10-kelime-zeyli-shorts-package.json > /tmp/20-mektup-10-kelime-zeyli-shorts-package-check.json
python3 -m json.tool docs/video-tests/shorts/20-mektup-10-kelime-zeyli/metadata/20-mektup-10-kelime-zeyli-shorts-metadata.json > /tmp/20-mektup-10-kelime-zeyli-shorts-metadata-check.json
```

JS syntax için:

```bash
node -e "new Function(require('fs').readFileSync('docs/video-tests/inputs/20-mektup-10-kelime-zeyli-landscape-load-input.js', 'utf8')); console.log('Landscape JS OK')"

for f in docs/video-tests/shorts/20-mektup-10-kelime-zeyli/load-inputs/*.js; do
...
```bash
grep -RniE "bu metin|metnin|metinde|metne göre|metnin deyimiyle|bu noktada|şu cümle|verilen cevap|verilen örnek|yazar burada|kitap burada|burada anlatılmak istenen|bu bölüm" docs/video-tests/inputs/20-mektup-10-kelime-zeyli* docs/video-tests/shorts/20-mektup-10-kelime-zeyli || true
```
