# Görev: 21. Mektup video üretim scaffold dosyalarını doldur
...
```bash
python3 -m json.tool docs/video-tests/inputs/21-mektup-evdeki-sessiz-bereket-landscape-full-video.json > /tmp/21-mektup-evdeki-sessiz-bereket-landscape-json-check.json
python3 -m json.tool docs/video-tests/metadata/21-mektup-evdeki-sessiz-bereket-landscape-metadata.json > /tmp/21-mektup-evdeki-sessiz-bereket-landscape-metadata-check.json
python3 -m json.tool docs/video-tests/shorts/21-mektup-evdeki-sessiz-bereket/21-mektup-evdeki-sessiz-bereket-shorts-package.json > /tmp/21-mektup-evdeki-sessiz-bereket-shorts-package-check.json
python3 -m json.tool docs/video-tests/shorts/21-mektup-evdeki-sessiz-bereket/metadata/21-mektup-evdeki-sessiz-bereket-shorts-metadata.json > /tmp/21-mektup-evdeki-sessiz-bereket-shorts-metadata-check.json
```

JS syntax için:

```bash
node -e "new Function(require('fs').readFileSync('docs/video-tests/inputs/21-mektup-evdeki-sessiz-bereket-landscape-load-input.js', 'utf8')); console.log('Landscape JS OK')"

for f in docs/video-tests/shorts/21-mektup-evdeki-sessiz-bereket/load-inputs/*.js; do
...
```bash
grep -RniE "bu metin|metnin|metinde|metne göre|metnin deyimiyle|bu noktada|şu cümle|verilen cevap|verilen örnek|yazar burada|kitap burada|burada anlatılmak istenen|bu bölüm" docs/video-tests/inputs/21-mektup-evdeki-sessiz-bereket* docs/video-tests/shorts/21-mektup-evdeki-sessiz-bereket || true
```
