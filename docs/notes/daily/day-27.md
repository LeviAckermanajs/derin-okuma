# Day-27 — Vesvese Video Paketi

`Vesvesenin Mahiyeti, Kalbin Beş Yarası ve Beş Merhem` yazısı için video üretim scaffold dosyaları gerçek içerikle dolduruldu.
...
- JSON parse, JS syntax ve yasaklı kalıp kontrolleri başarıyla çalıştırıldı
- `video:validate --report` PASS verdi
...
Shorts için her `short-XXX-load-input.js` dosyası ayrı job olarak gönderilir. Batch kullanılacaksa önce `video:validate`, ardından `video:batch` çalıştırılır.
